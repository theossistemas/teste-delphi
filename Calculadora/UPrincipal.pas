...
var
  ...
  vFlag : string; // Sinaliza operações
  vUOper: string; // Armazena último operador utilizado
  var vbotao : string; // Valor da String para os botões
...
begin
  ...
  Luv1.Caption := '';     // último valor digitado n1
  Luv2.Caption := '';     // último valor digitado n2
...
begin
  res_ori := 0;
   ...
   if (Luop.Caption = 'x') then   // Para Multiplicação
   begin
     if (vFlag <> '=') then
      begin
   ...
   begin
    if (LVisor.Caption <> '0') then
     ...
     begin
       ShowMessage('Divisões por 0 "ZERO" não são permitidas');
       exit;
     end;
   end;
  ...
  if (res_ori-Int(res_ori) <= 0) then  // Formata a saída para o usuário
   begin
